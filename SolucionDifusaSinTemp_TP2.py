...
# Conjuntos Z (Sintonía fina ajustada al rango operativo real)
z_input['MUY_NEG'] = fuzz.trapmf(z_input.universe, [-100, -100, -5, -1])
z_input['NEG'] = fuzz.trimf(z_input.universe, [-5, -1, 0])
z_input['ZERO'] = fuzz.trimf(z_input.universe, [-1, 0, 1])
z_input['POS'] = fuzz.trimf(z_input.universe, [0, 1, 5])
z_input['MUY_POS'] = fuzz.trapmf(z_input.universe, [1, 5, 100, 100])
...
def simular_difuso(T_media, T_amplitud, metodo_desborrosificacion):
    # Se asigna el método de desborrosificación (Centroide o MOM) antes de simular
    ...
    for i in range(n_steps - 1):
        ...
        # Traducción a constante de tiempo y Euler
...
def main():
    ...
    metodos = [
        ('centroid', 'Centroide (COG) - Movimiento Suave'),
        ('mom', 'Máximo (MOM) - Movimiento a Saltos')
    ]
    
    print("Iniciando simulaciones comparativas (esto puede tomar unos 15 segundos)...")
    ...
    for nombre_caso, t_med, t_amp in escenarios_tp:
        ...
        for col, (codigo_metodo, titulo_metodo) in enumerate(metodos):
            ...
            # Gráficos de Temperatura (Fila Superior)
            ax_temp = axs[0, col]
    ...
    print("Simulaciones completadas. Mostrando las 3 ventanas...")
    plt.show()
